...
namespace MQTT_SERVER
{
    public class App : BackgroundService
    {
        ...
        public App(ILogger<App> logger, AppSettings appSettings  )
        {
          ...
          .WithUrl(new Uri("http://10.4.5.132:1112/ess-hub"))
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                _connection.On<string, string , int>("Stop", (message, line,ID) =>
                {
                    Console.ForegroundColor = ConsoleColor.Yellow;
                    string newMessage = $"#### ### Listen from angular: {message}: {line} : {ID}";
        ...
        private async Task RecevicedMyTopic(MqttApplicationMessageReceivedEventArgs e)
        {
            ...
            try
            {
                ...
                await _connection.InvokeAsync("Welcom", "1", MyTopic.Line , MyTopic.boxCount);
            }
            catch (Exception ex)
            {
                throw;
